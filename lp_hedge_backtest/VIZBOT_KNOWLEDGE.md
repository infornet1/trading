...
- **Inverted SL auto-correction (2026-06-15)**: `signal_executor.py` detects when a signal's SL is on the wrong side of entry (e.g. SL below entry for a SHORT — common when the channel swaps "Stoploss" and "Target" labels). When detected and TP1 is on the correct side, executor auto-swaps SL↔TP1 and logs `⚠️ [Executor] {symbol}: inverted SL detected — auto-swapped SL↔TP1`. If no valid swap target exists, rejects with a clear `"Inverted SL"` error (not the confusing "beyond stoploss" message). Skipped when user supplied an explicit SL override. First triggered by DOT and ETC signals on 2026-06-15 (channel labeled SL below entry for both SHORTs).
