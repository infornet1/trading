...
- **SMTP health check at startup (2026-06-15)**: listener tests SMTP credentials at boot via `check_smtp()`. Logs `✅ SMTP health check OK` or `🚨 SMTP health check FAILED` + immediate Telegram push to admin chats if broken. Catches expired App Passwords before the first real event fires.
- **Telegram fallback for email (2026-06-15)**: `send_signal_email()` now pushes to admin Telegram chats (`ADMIN_TELEGRAM_CHAT_IDS` in api/.env) when SMTP fails — via `_tg_fallback()` using `@vizniago_bot`. Labeled `🔴 [Signal Lab — email DOWN]`. Both channels (email + Telegram) active simultaneously; Telegram fires only on email failure. Previously, a broken App Password silenced all notifications with no fallback.
