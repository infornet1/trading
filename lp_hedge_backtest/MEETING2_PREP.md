...
### Strategic Depth Enhancements — Gap Analysis (2026-05-31)

*Items identified from fresh V2 behavior review + live trade post-mortems + external strategy research (2026-05-31). Priority ordered.*

**Architecture ceiling (research finding):** The LP position is structurally equivalent to being short a straddle — it loses to volatility in both directions. A single linear perp short hedges only delta (first-order). Gamma (second-order IL from volatility itself) remains permanently unhedged with this architecture. Options or power perpetuals would fix gamma but are not available on Hyperliquid. All improvements below operate within this ceiling.

| ID | Item | Effort | Why | Status |
|---|---|---|---|---|
| M2-48 | Dynamic delta sizing — resize short continuously as ETH moves through range | High | **Research finding (2026-05-31).** Current fixed 50% hedge ratio is only correct at the geometric midpoint of the range (√(lower × upper) ≈ $2,205 for Config 17). Formula: `Δ = L · (1/√P − 1/√P_upper)`. At the lower bound the true LP delta is ~100% of X_max — the current 50% short leaves ~50% of IL exposure unhedged at exactly the worst moment. At the upper bound, delta is ~0 — the current short wastes capital on a position that hedges nothing. arXiv:2603.19716 (Hane 2026) confirms optimal range under liquidation constraints is 50–70% of theoretical full-delta — current static 50% is at the lower edge and is non-adaptive. Implementation: recalculate `short_size_ETH = L · (1/√P − 1/√P_upper)` at each poll; when computed size differs from current open HL position by >5%, adjust via HL modify-order. Requires tracking L (already fetched on startup). This is the highest-theoretical-impact change to the entire strategy. | 🔲 |
| M2-49 | ATR-based breakeven trigger — replace fixed 1% with `max(1.0%, 1.5 × ATR(14))` | Low | **Research finding (2026-05-31).** ETH 14-period ATR on 1h candles typically runs 0.5–2.5%. The `BREAKEVEN_PCT=1.0%` was firing inside one ATR unit in volatile sessions — normal noise closed positions at breakeven before the trend developed. Config 17 evidence: May 27–28 (+4.464%) and May 14–17 (+5.6%) big wins only ran because they escaped the BE zone; tighter BE in high-vol would have pinched them to zero. **Implemented 2026-05-31:** `_compute_atr_breakeven(entry_price)` fetches 16×1h HL candles, computes ATR(14), returns `max(BREAKEVEN_PCT, ATR_MULT_BE × atr_pct)`. Called at `open_hedge()`, stored as `self._effective_breakeven_pct`. Logged in `hedge_opened` details as `breakeven_pct`. Orphan recovery uses static default (no ATR context at recovery). Tunable via `ATR_PERIOD` (default 14) and `ATR_MULT_BE` (default 1.5) env vars. Live ATR test at deploy time: $6.74/hr (0.33%, calm market) → 1.5×ATR=0.5% < 1% floor → static 1% used correctly. Startup log: `Breakeven: 1.0% base (ATR-adaptive, 1.5×ATR14)`. | ✅ Done 2026-05-31 |
| M2-41 | Candle-body + consecutive-poll confirmation for `below_range` trigger | Low | Layer 1+2 from NFT #5374616 wick case study. **Live data check (2026-05-31, Config 17 / NFT 5413901, 54 trades):** below_range is only 5/54 trades (9%) — ETH stayed above range until late May. Of those 5, only 1 is a wick candidate (May 24: entered $2061.45, SL hit in 24 min, +1.56% bounce). Layer 2 consecutive-poll check (60s) would have blocked it. The other 4 lasted hours = genuine breakdowns, no filter needed. **Real impact: 1/54 trades = ~2%.** Layer 1 (candle body fetch): would have blocked the same trade. Valid improvement, downgraded to Low priority based on live data. Still worth implementing alongside M2-47 (same candle fetch). | 🔲 |
| M2-42 | ATR-adaptive SL floor at trigger time | Low | Originally scoped for Config 20 (0.1% SL → whipsawed). **Live data check (2026-05-31, Config 17):** Config 17 already at sl_pct=1.5%. ATR 1h ETH in 2026 ≈ 0.6–1.7%, so 1.5×ATR ≈ 0.9–2.5% — current 1.5% is already in the middle of ATR range. Zero impact on Config 17. Remains valid for new configs where users set tight SL. Batch with M2-41 (same candle fetch) when building. | 🔲 |
| M2-47 | From-above entry distance gate — skip entry if ETH is >X% below upper_bound | Medium | **Identified 2026-05-31 from live data analysis of Config 17 (54 trades).** Many from_above entries in May 2026 happen at prices 9–15% below the upper_bound ($1998, $2092, $2114 etc.) because `price_was_above` was set in April when ETH was above $2347 and stays True while ETH drifts down. At these prices the LP has mostly converted to ETH — the "fell back into range from above" hedging thesis no longer applies, yet the bot enters from_above because guard clears + price is already below upper_trigger. **Implemented 2026-05-31:** `MAX_FROM_ABOVE_DIST_PCT = 5.0` env var (default). Guard check: `price >= upper_bound × (1 - MAX_FROM_ABOVE_DIST_PCT / 100)`. If price is >5% below upper_bound, from_above entry skipped with log `⏭️ [M2-47] from_above skipped`. below_range trigger remains active unaffected. Startup log shows `M2-47 gate: ≤5.0% below upper`. For Config 17 (upper=$2347.28): gate floor = $2229.92 — blocks entries when ETH is below ~$2230 on a stale arm. | ✅ Done 2026-05-31 |
| M2-43 | IL attribution per trade — log `il_offset_pct` on close events | Medium | The core strategy thesis is: LP fees + hedge P&L ≥ IL incurred. The bot logs hedge P&L but never asks "how much IL did I actually offset on this trade?" Without per-trade attribution, live validation of the thesis is impossible. Fix: on hedge close event, compute LP position value delta using Uniswap v3 math (already built in `dashboard.js` — `computePositionAmounts()`) from `hedge_opened` to close timestamp. Write `il_offset_pct` + `fees_earned_est` to `bot_events.details`. Show in forensic event history (e.g. "🛡️ −2.3% IL offset"). Potential VIZNAGO differentiator — no other platform shows per-trade hedge attribution. | 🔲 |
| M2-44 | Funding rate awareness — log cost while hedge is open | Low | When a short is open, HL funding accumulates silently. In strong bull moves shorts can pay longs, eroding hedge P&L without any alert. FURY has a funding rate gate (Gate 6: > +0.05% → SHORT only); LP Defensor has nothing. **Phase 1 (low effort):** on `hedge_opened`, fetch and log current 1h funding rate in `details`. On close event, log estimated cumulative funding paid/received. **Phase 2 (optional):** `USE_FUNDING_GATE=True` env var — warn/suppress entry if funding is adverse beyond threshold. Phase 1 alone is a pure observability improvement, zero risk, no behavior change. | 🔲 |
| M2-45 | Out-of-range duration advisory (extend LP reconciler) | Low | LP reconciler catches burned/zero-liquidity NFTs but never tracks how long a pool has been out of range. A pool OOR for 24h+ earns zero fees and fully converts to one asset — significant opportunity cost. Fix: track `out_of_range_since` timestamp in `data_cache/reconciler_state.json`. After configurable N hours (default 24h), fire new `lp_oor_advisory` event + email suggesting rebalance or close. Zero bot restart (reconciler runs inside API process). Requires adding `lp_oor_advisory` to MariaDB enum + SQLAlchemy `BotEvent.event_type`. | 🔲 |
| M2-46 | Cross-pool hedge overlap warning | Low | When two LP configs share the same HL wallet and both range conditions trigger simultaneously, two independent shorts stack on the same account without any coordination. Fix: at `open_hedge()` time, check if any other active `bot_config` with the same `hl_wallet_addr` already has `hedge_active=True` (in-memory check via BotManager state, no API call). If so, log `overlap_warning: X bots on this wallet` in `hedge_opened` details. Observation only — no suppression (user may deliberately use different wallets per pool). | 🔲 |
...
*Last updated: 2026-05-31 — M2-47 ✅ live (from-above distance gate, `MAX_FROM_ABOVE_DIST_PCT=5.0`). M2-49 ✅ live (ATR-adaptive breakeven). M2-48 added (dynamic delta sizing, highest theoretical impact). M2-47 added (from-above distance gate, highest-impact per live data) and deployed same session. M2-41/42 downgraded to Low after live data analysis (Config 17, 54 trades). M2-41–M2-46 added (strategic depth gap analysis). SL-P7/7b/7c ✅ copy trading auto-execute live. M2-39 permanent DB enum fix `4cdf0d3`. M2-34 ✅ M2-28 ✅ M2-30 ✅ M2-6 ✅ M2-7 ✅ M2-16 (UI) ✅ T1-3 ✅ T2-6 ✅ M2-9 ✅ M2-13 ✅ M2-21 ✅ M2-39 ✅ M2-40 ✅ all live-validated. M2-29 deferred.*
