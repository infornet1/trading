...
| M2-39 | Circuit breaker: pause after N consecutive SL hits | Medium | **Identified 2026-04-21 via trade audit on NFT 5435626.** On Apr 19 ETH choppy session ($2,332–$2,341), bot opened 7 consecutive SHORTs in 85 min — all stopped out by the native SL (0.1% = ~$2.34 room vs ~$8 ETH oscillation). Cost: -$2.27 USDC in losses + commissions before the big winner (#8) recovered. **Fix:** after N consecutive `external_close` or `sl_hit` events without a breakeven or winner, pause re-entry for X minutes. Recommended defaults: **N=3 stops → 20 min pause**. Reset counter on breakeven hit or clean profitable close. Mirrors FURY bot circuit breaker logic (`fury_circuit_breaker` event). New env vars: `CIRCUIT_BREAKER_LOSSES` (default 3), `CIRCUIT_BREAKER_PAUSE_MIN` (default 20). Requires bot restart. Estimated savings on Apr 19 session: ~$0.87 USDC (4–5 stops avoided). | 🔲 Post-meeting |
| M2-40 | Cooldown period after external_close re-arm | Low | **Identified 2026-04-21 via trade audit on NFT 5435626.** After a native SL fires and bot detects `external_close`, reentry guard clears in 31–61 seconds in choppy markets — bot re-enters almost immediately into the same adverse conditions. **Fix:** add a minimum cooldown delay after any `external_close` before allowing re-arm, regardless of guard logic. Recommended default: **`REARM_COOLDOWN_SEC = 300` (5 min)**. Zero UX impact — bot card shows idle during cooldown. No changes to guard logic or SL params — purely additive. Lowest-risk improvement of the three options researched. Requires bot restart. Works as first line of defense; pairs well with M2-39 circuit breaker as second line. | 🔲 Post-meeting |
...
*Last updated: 2026-04-22 — M2-9 native TP implemented and deployed to `live_hedge_bot_v2.py`*
